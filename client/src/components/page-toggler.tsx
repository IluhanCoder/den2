...
const PageToggler = ({currentPage, setPage}: LocalParams) => {
    ...
    if(pagesAmount > 1)
        return <div>
            <div>
                { currentPage > 0 && 
                    <button type="button" onClick={previousPageHandler} className={buttonStyle}>назад</button> 
                }
                { currentPage + 1 < pagesAmount &&
                    <button type="button" onClick={nextPageHandler} className={buttonStyle}>вперед</button>
                }
            </div>
