...
const RangeSelector = ({label, from, setFrom, to, setTo, min, max}: localParams) => {
    ...
    const changeToHandler = (event: any) => {
        ...
        if(from !== undefined) {
            if(newValue < minValue || newValue > maxValue) return;
            if(newValue < from + 1) { setTo(Number(from) + 1); return; }
        }
        setTo(newValue)
    }
    ...
    return <div className={style}>
        <div>{label}:</div>
        <label>Від:</label><input type="number" onChange={e => changeFromHandler(e)} value={from ?? 0}/>
        <label>До:</label><input type="number" onChange={e => changeToHandler(e)} value={to ?? 0}/>
    </div>
}
