...
export default new (class ProductService {
  ...
  async filterProducts(filterQuery: IQuery, offset: number, sort: string) {
    try {
      ...
      if (filterQuery.nameString && filterQuery.nameString.length > 0)
        query.name = { $regex: filterQuery.nameString };
      if (filterQuery.descString && filterQuery.descString.length > 0)
        query.desc = { $regex: filterQuery.descString };
      if (filterQuery.category && filterQuery.category.length > 0)
        query.category = filterQuery.category;
      if (filterQuery.status) query.status = filterQuery.status;
      if (filterQuery.quantityStart && filterQuery.quantityEnd) {
        query.quantity = {
          $gte: filterQuery.quantityStart,
          $lte: filterQuery.quantityEnd,
        };
      }
      if (filterQuery.priceStart && filterQuery.priceEnd) {
        query.price = {
          $gte: filterQuery.priceStart,
          $lte: filterQuery.priceEnd,
        };
      }
      const products = (
        await $api.post("/filter-products", { query, offset, sort })
      ).data;
      return products;
    } catch (error) {
      throw error;
